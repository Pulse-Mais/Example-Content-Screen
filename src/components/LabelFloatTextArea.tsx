...
export function LabelFloatTextarea({ _title, name, register, labelBackground, error, ...rest } : ILabelFloatProps) {
    return (
        <div className="relative">
            <textarea id={name || register.name} className={twMerge("resize min-h-11 max-h-72 min-w-28 w-full max-w-full text-zinc-700 text-base bg-transparent border-1 border-zinc-300 rounded outline-none p-2 focus:border-midnight-blue-950 peer", rest.className, error && "border-red-500 focus:border-red-500")} placeholder=" " {...rest} {...register}></textarea>
